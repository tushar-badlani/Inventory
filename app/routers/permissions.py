...
async def create_permission(permission: schemas.PermissionCreate, db: Session = Depends(get_db)):
    ...
    db_permission = models.Permission(user_id=permission.user_id, event_id=permission.event_id, approver_id=permission.approver_id, permission_type=permission.permission_type, description=permission.description)
